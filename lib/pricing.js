...
const { crossCheckPrices, arbitrate } = require('./pricecheck');

const PRICE_API = 'https://lite-api.jup.ag/price/v3';
const CROSS_CHECK_DISABLED = process.env.PRICECHECK_DISABLED === '1';
...
const stats = { requests: 0, batchOk: 0, batchFail: 0, resolved: 0, unresolved: 0, arbitrated: 0 };
...
async function getPrices(mints) {
  ...
  // Contre-verification par une source independante. Jamais bloquante.
  let reference = new Map();
  if (!CROSS_CHECK_DISABLED) {
    try {
      const checked = await crossCheckPrices(unique);
  ...
  for (const m of unique) {
    ...
    const verdict = arbitrate(m, resolved.price, reference);
    let entry = resolved;

    if (verdict) {
      if (verdict.price !== resolved.price) {
        // La reference l'emporte : le prix retenu devient celui de DexScreener.
        entry = { ...resolved, price: verdict.price, status: STATUS.FRESH, ageMs: 0, fetchedAt: after };
        stats.arbitrated++;
        console.warn(
          `[prices] ${m} : divergence ${(verdict.divergence === null ? 0 : verdict.divergence * 100).toFixed(1)}% ` +
          `(jupiter=${resolved.price} dexscreener=${verdict.reference.price}, ` +
          `liquidite $${Math.round(verdict.reference.liquidityUsd)}) -> ${verdict.reason}`
        );
      } else {
        entry = { ...resolved };
      }
      entry.crossCheck = {
        ...
        referencePrice: verdict.reference.price,
        liquidityUsd: verdict.reference.liquidityUsd,
        dex: verdict.reference.dex,
        symbol: verdict.reference.symbol,
      };
    }

    if (entry.status === STATUS.UNPRICED) stats.unresolved++;
    else stats.resolved++;
    out.set(m, entry);
  }
