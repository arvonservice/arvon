...
function recordMatchResult(id, { mode, durationSec, pnlPct, result, staked = 0, netCash = 0, verification = null }) {
  const user = getUserById(id);
  if (!user) return;
  const s = user.stats;
  s.matches++;
  ...
  data.matchHistory.push({ userId: id, mode, durationSec, pnlPct, result, staked, netCash, verification, ts: Date.now() });
  persist();
}
