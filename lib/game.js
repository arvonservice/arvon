const { getWalletSnapshot, snapshotTotal, frozenValueRelativeTo } = require('./solana');
const db = require('./db');
      ...
      allPlayers.map(async (p) => {
        ...
        if (st.simulated) {
          ...
          try {
            ...
            const frozenValue = frozenValueRelativeTo(st.baselineSnapshot, st.lastSnapshot);
            const pnl = frozenValue - st.baseline;
            st.pnlPct = st.baseline > 0 ? round2((pnl / st.baseline) * 100) : 0;
            st.walletError = null;
          } catch (e) {
