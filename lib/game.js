const { getWalletSnapshot, snapshotTotal } = require('./solana');
const db = require('./db');
...
class GameEngine {
  ...
  async createMatch(mode, durationKey, participants, stakesMap) {
    ...
    for (const p of [...teamA, ...teamB]) {
      ...
      if (!p.isBot && p.wallet) {
        try {
          const snap = await getWalletSnapshot(p.wallet);
          const value = snapshotTotal(snap);
          if (value > 0) {
            baseline = value;
    ...
    await Promise.all(
      allPlayers.map(async (p) => {
        ...
        if (st.simulated) {
          ...
          try {
            const now = Date.now();
            const shouldRefetch = !st.lastSnapshotTime || (now - st.lastSnapshotTime) > 10000;

            if (shouldRefetch) {
              st.lastSnapshot = await getWalletSnapshot(p.wallet);
              st.lastSnapshotTime = now;
              st.lastSnapshotFresh = true;
            } else if (!st.lastSnapshot) {
              st.lastSnapshot = st.baselineSnapshot;
              st.lastSnapshotFresh = false;
            }

            // CRITICAL: Performance formula (no more frozen prices)
            // performancePct = ((currentEquity / initialEquity) - 1) * 100
            const currentEquity = snapshotTotal(st.lastSnapshot);

            if (st.baseline > 0) {
              st.pnlPct = round2(((currentEquity / st.baseline) - 1) * 100);
              if (shouldRefetch && st.lastSnapshotFresh) {
                const freshStr = st.lastSnapshot.timestamp && (Date.now() - st.lastSnapshot.timestamp < 1000) ? '🟢' : '🟡';
                console.log(`[${freshStr}] ${p.name}: baseline=$${st.baseline.toFixed(2)}, current=$${currentEquity.toFixed(2)}, pnl=${st.pnlPct}%`);
              }
            } else {
              st.pnlPct = 0;
            }

            // Track unpriced assets
            st.unpricedAssets = st.lastSnapshot.unpricedMints || [];
            if (st.unpricedAssets.length > 0 && shouldRefetch) {
              console.warn(`[!] ${p.name} has ${st.unpricedAssets.length} unpriced token(s), excluded from PnL calculation`);
            }
            st.walletError = null;
          } catch (e) {
            st.walletError = 'Wallet injoignable, derniere valeur connue';
            console.error(`[tickMatch] Error calculating PnL for player ${p.name}:`, e.message);
          }
        }
